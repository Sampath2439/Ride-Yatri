...
export default function Support() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setEmergencyOn(false), 10000);
    return () => clearTimeout(timer);
  }, [emergencyOn]);

  const showEmergencyOnToast = () => {
    setEmergencyOn(true);
    toast({
      title: "Emergency Mode On",
      description: "Women Safety & SOS enabled",
      duration: 3000,
      action: (
        <ToastAction altText="Disable" onClick={() => setEmergencyOn(false)}>
          Disable
        </ToastAction>
      ),
    });
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {emergencyOn && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[2000]">
          <div className="bg-red-600 text-white px-6 py-4 rounded-xl shadow-2xl border border-red-500 flex items-center gap-4">
            <span className="relative flex h-4 w-4">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75"></span>
              <span className="relative inline-flex rounded-full h-4 w-4 bg-white"></span>
            </span>
            <div className="text-center">
              <div className="text-xl font-extrabold tracking-wide">EMERGENCY MODE ACTIVE</div>
              <div className="text-xs opacity-90">Prioritizing Women Safety & SOS features</div>
            </div>
            <button
              onClick={() => setEmergencyOn(false)}
              className="ml-4 bg-white/15 hover:bg-white/25 transition-colors text-white px-3 py-1.5 rounded-md text-xs font-semibold border border-white/20"
            >
              Disable
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-pink-500">
              ...
              <button
                onClick={showEmergencyOnToast}
                className="w-full mt-4 bg-pink-500 hover:bg-pink-600 text-white font-semibold py-3 px-4 rounded-lg transition-colors"
              >
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-red-500">
              ...
              <button
                onClick={showEmergencyOnToast}
                className="w-full mt-4 bg-red-500 hover:bg-red-600 text-white font-bold py-4 px-4 rounded-lg transition-colors shadow-lg"
              >
                🚨 EMERGENCY SOS
              </button>
            </div>
